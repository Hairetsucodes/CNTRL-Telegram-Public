...
def word_results(chatId, userId):
    ...
    try:
        words = (
            db_session.query(UserWordCount)
            .filter(UserWordCount.chatId == chatId, UserWordCount.userId == userId)
            .all()
        )
...
def add_word_count(chatId, word, userId):
    ...
    try:
        word_count = (
            session.query(WordCounter)
            .filter(WordCounter.word == word, WordCounter.chatId == chatId)
            .first()
        )
        if not word_count:
            ...
            session.commit()
        user_word_count = (
            session.query(UserWordCount)
            .filter(
                UserWordCount.word == word,
                UserWordCount.userId == userId,
                UserWordCount.chatId == chatId,
            )
            .first()
        )
        if not user_word_count:
            new_user_word_count = UserWordCount(
                word=word,
                userId=userId,
                count=1,
                chatId=chatId,
                username=(
                    session.query(User).filter(User.id == userId).first()
            ...
            session.query(UserWordCount).filter(
                UserWordCount.word == word,
                UserWordCount.userId == userId,
                UserWordCount.chatId == chatId,
            ).update({UserWordCount.count: UserWordCount.count + 1})
            session.commit()
    except Exception as e:
        print(f"An error occurred: {e}")
        session.rollback()
...
def top_five_leaderboard(chatId):
    ...
    try:
        ...
        for word in words:
            users = (
                db_session.query(UserWordCount)
                .filter(UserWordCount.word == word.word, UserWordCount.chatId == chatId)
                .order_by(UserWordCount.count.desc())
                .limit(5)
                .all()
            )
...
def last_youtube(chatId):
    ...
    try:
        chat = db_session.query(Chat).filter(Chat.chatId == chatId).first()
        if not chat or chat.lastYT is None:
            return "Sorry there is no youtube link in this chat."
        return str(chat.lastYT)
    except Exception as e:
        print(f"An error occurred: {e}")
        db_session.rollback()
        return "Sorry there is no youtube link in this chat."
...
def get_last_x_chat_messages(chatId, x):
    ...
    try:
        ...
        messages = (
            ...
            .filter(ChatMessages.chatId == chatId)
            .all()[-x:]
        )
...
def check_b7(userId):
    ...
    try:
        ...
        if not user:
            return False
        return bool(user.blacklisted == 1)
    except Exception as e:
        print(f"An error occurred: {e}")
        db_session.rollback()
        return False
...
def last_five_messages(chatId, userId):
    ...
    try:
        messages = (
            db_session.query(ChatMessages)
            .filter(ChatMessages.chatId == chatId, ChatMessages.userId == userId)
            .all()[-5:]
        )
